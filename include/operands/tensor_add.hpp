#ifndef TENSOR_ADD_HPP
#define TENSOR_ADD_HPP
...
namespace synaptic
{
    ...
    public:
        ...
        void backward(std::shared_ptr<tensor<type>> operand1, std::shared_ptr<tensor<type>> grad_output, std::shared_ptr<tensor<type>> operand2 = nullptr);

    private:
        std::shared_ptr<tensor<type>> cpu_forward(std::shared_ptr<tensor<type>> operand1, std::shared_ptr<tensor<type>> operand2);
        void cpu_backward(std::shared_ptr<tensor<type>> operand1, std::shared_ptr<tensor<type>> grad_output, std::shared_ptr<tensor<type>> operand2 = nullptr);
        std::shared_ptr<tensor<type>> general_forward(std::shared_ptr<tensor<type>> operand1, std::shared_ptr<tensor<type>> operand2);
        void general_backward(std::shared_ptr<tensor<type>> operand1, std::shared_ptr<tensor<type>> grad_output, std::shared_ptr<tensor<type>> operand2 = nullptr);
    };
}
