...
{
    if(this->device == devices::none)
    {
        std::cout << "Device not set for tensor_add" << std::endl;
    }
    if (this->device == devices::cpu)
    {
...
template <typename type>
void tensor_add<type>::backward(std::shared_ptr<tensor<type>> operand1, std::shared_ptr<tensor<type>> grad_output, std::shared_ptr<tensor<type>> operand2)
{
    // Get the appropriate implementation based on the current device
    std::cout << "Performing backward pass for tensor addition" << std::endl;
    auto impl = get_impl_selector_backward();
    // Call the selected implementation
    impl(operand1, grad_output, operand2);
}
...
template <typename type>
void tensor_add<type>::general_backward(std::shared_ptr<tensor<type>> operand1, std::shared_ptr<tensor<type>> grad_output, std::shared_ptr<tensor<type>> operand2)
{
    ...
    {
        operand1->grad[i] += grad_output->grad[i];
        operand2->grad[i] += grad_output->grad[i];
    }
}

template <typename type>
void tensor_add<type>::cpu_backward(std::shared_ptr<tensor<type>> operand1, std::shared_ptr<tensor<type>> grad_output, std::shared_ptr<tensor<type>> operand2)
{
    return;
}
